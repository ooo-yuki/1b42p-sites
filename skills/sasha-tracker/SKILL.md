...
Три места: отображаемое имя (map ключ→имя), цвет (map ключ→hex — hex обязан быть новым, без дублей палитры), ключ в конец `ORDER` (`ORDER` также крутит пики онлайна — без ключа сайт не покажется нигде). Если папка сайта не совпадает с ключом (как `laiv42` → `42laiv.bratuxa.zomb.top`), в `collect.mjs` маппить явно.
