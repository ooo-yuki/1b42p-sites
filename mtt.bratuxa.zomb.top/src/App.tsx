...
async function loadStats(): Promise<void> {
  ...
  const [devBanHours, setDevBanHours] = useState(0);
  const [devBanMins, setDevBanMins] = useState(0);
  ...
  return (
    <>
        ...
        >
          ...
          <div className="board" id="mapSec">
            ...
            <div className="mapRow">
              {MAPS.filter((m) => (m.id !== 'szeged' || canSee(authed, devUnlocked)) && (m.id !== 'forest' || canSee(authed, devUnlocked)) && (m.id !== 'blender' || canSee(authed, devUnlocked)) && m.id !== 'boss').map((m) => (
                <button
                  key={m.id}
                  id={`map-${m.id}`}
          ...
          <div className="board" id="bossSec">
            ...
            <div className="mdesc">Мировой босс 3500 HP на круглой арене: 25 огненных кругов (50), рука 25, прыжок с меткой 10м (60). Кубков за него нет — награда 💰 7000 фантиков делится между всеми, кто наносил урон за спавн (пропорционально урону). Не убили за 30 мин — босс появляется заново, и награды никому нет. Макс 7 бойцов — умер, и тебя выкинуло до следующего респауна (30 мин).</div>
            {roomsList.filter((r) => r.mode === 'boss').length > 0 ? roomsList.filter((r) => r.mode === 'boss').map((r) => {
              const cap = modeCap(r.mode);
              ...
              return createPortal(
                <div className="devModalOverlay" onClick={(e) => { if (e.target === e.currentTarget) setDevUserMenu(null); }}>
                  <div className="devModalBox">
                    ...
                    {!x.ipBlocked && x.ip && <>
                      <div style={{ fontSize: 12, color: '#aaa', margin: '8px 0 4px' }}>Время бана по IP:</div>
                      <div style={{ display: 'flex', gap: 6, marginBottom: 8, alignItems: 'center' }}>
                        <label style={{ fontSize: 12, color: '#888' }}>
                          <input type="number" min={0} max={365} value={devBanDays} onChange={(e) => setDevBanDays(+e.target.value)} style={{ width: 40, background: '#0a0806', border: '1px solid #5a4a22', color: '#fff', borderRadius: 4, padding: '4px 2px', fontFamily: 'inherit', fontSize: 12 }} /> д
                        </label>
                      ...
                      <button className="wbtn" onClick={async () => {
                        const totalMin = devBanDays * 1440 + devBanHours * 60 + devBanMins;
                        const label = totalMin > 0 ? `${devBanDays}д ${devBanHours}ч ${devBanMins}мин` : 'навсегда';
                        if (!window.confirm(`Заблокировать IP ${x.ip} (${x.login}) на ${label}?\nИгрок не сможет играть и регистрироваться.`)) return;
                        await fetch('/api/dev/block-ip', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: token(), login: x.login, duration: totalMin }) });
                        setDevUsers((u) => (u ?? []).map((y) => y.login === x.login ? { ...y, ipBlocked: true, ipExpires: totalMin > 0 ? Date.now() + totalMin * 60_000 : 0 } : y));
                        setDevUserMenu(null);
                      }}>🚫 Заблокировать по IP</button>
                    </>}
                    {x.ipBlocked && x.ip && <div style={{ fontSize: 12, color: '#aaa', marginBottom: 8 }}>
                      {x.ipExpires > 0
                        ? `🚫 IP заблокирован до ${new Date(x.ipExpires).toLocaleString()}`
                    ...
                    {x.ipBlocked && x.ip && <button className="wbtn" onClick={async () => {
                      ...
                      setDevUsers((u) => (u ?? []).map((y) => y.login === x.login ? { ...y, ipBlocked: false, ipExpires: 0 } : y));
                      setDevUserMenu(null);
      ...
      <div id="fogOverlay" style={{ opacity: hud.fog }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '45vh', background: 'linear-gradient(to bottom, rgba(10,2,28,0.98), rgba(10,2,28,0))' }} />
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '45vh', background: 'linear-gradient(to top, rgba(10,2,28,0.98), rgba(10,2,28,0))' }} />
        <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: '42vw', background: 'linear-gradient(to right, rgba(10,2,28,0.98), rgba(10,2,28,0))' }} />
        <div style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: '42vw', background: 'linear-gradient(to left, rgba(10,2,28,0.98), rgba(10,2,28,0))' }} />
      </div>
