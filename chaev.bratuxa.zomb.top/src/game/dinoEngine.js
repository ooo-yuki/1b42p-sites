// Дино-движок 42: чистая логика без DOM/canvas — рендер в DinoGame.jsx.
...
export function jumpDino(s) {
  ...
  if (s.dino.jumps < 2) {
    s.dino.vy = s.dino.jumps === 0 ? -12 : -10;
    s.dino.jumps++;
  }
}
...
export function stepDino(s, rand = Math.random) {
  let event = null;
  s.t++;
  if (s.alive) {
    s.dino.vy += 0.7;
