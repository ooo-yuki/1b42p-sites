...
export interface ZapoiState {
  ...
  deals: number;
  /** Уровень запоя 0–4 (Похмелье → Делирий). */
  lvl: number;
  /** Суммарно нагнано бухла за забег — прогресс открытия уровней. */
  earned: number;
  /** Секунд на текущем уровне — от него угасает доход. */
  lvlSec: number;
  /** Предложенный «с рук» скрытый артефакт (взять/пропустить). */
  offer: string | null;
  /** Уровни, где оффер уже предлагали (по одному за уровень). */
  offerDone: Record<string, number>;
