...
describe('chess king safety (ФИДЕ 1.4.1)', () => {
  test('король не ест фигуру под защитой пешки', () => {
    // чёрные пешки e6 + d5 (d5 защищена), белый Крe4, ход белых
    const st = fromRows([
      '....k...',
      '........',
      '....p...',
    ...
    const targets = legalFrom(st, W, 36).map(m => m.to); // Крe4
    expect(targets).not.toContain(27); // d5 под боем e6 — брать нельзя
  });
  test('короля вообще нельзя съесть: ладья не бьёт короля', () => {
    // белая Ла1, чёрный Крa8, белые короли h2, ход белых
    const st = fromRows([
      'k.......',
      '........',
      '........',
    ...
    const targets = legalFrom(st, W, 56).map(m => m.to); // Ла1
    expect(targets).not.toContain(0); // a8 с королём — не цель
    expect(applyMove(st, W, { from: 56, to: 0 }).ok).toBe(false);
  });
  test('чёрная пешка даёт шах по-настоящему (d2 бьёт e1)', () => {
    const st = fromRows([
      '....k...',
      '........',
      '........',
  ...
  test('белая пешка даёт шах по-настоящему (d7 бьёт e8)', () => {
    const st = fromRows([
      '....k...',
      '...P....',
      '........',
