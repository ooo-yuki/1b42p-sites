...
import {
  DEFAULT_BET, DEFAULT_CHOICE, MIN_STAKE, SPIN_MS,
  choiceLabel, nextAngle, settle, spinPocket, validateRoulette,
} from './roulette/data';
...
/* ЗАЛ «РУЛЕТКА» — салон. Оркестрация: колесо → сукно → журнал.
   ...
   Исход решает кубик В НАЧАЛЕ спина, колесо докручивается под него
   одной CSS-транзицией (2 рендера за спин, без gsap и тиков). */
...
export default function Roulette({ api }: { api: Api }): JSX.Element {
  ...
  const [ballPocket, setBallPocket] = useState<number | null>(null);
  ...
  const spin = (): void => {
    ...
    setSpinning(true);
    setWon(null);
    setAngle(a => nextAngle(a, n));
    setBallPocket(n);
    if (api.reduced) {
      finish(n, stake);
      return;
    }
    // цок шарика — только звук, без единого рендера
    for (let i = 0; i < 4; i++) {
      later(() => ballTick(i / 4), (SPIN_MS / 5) * (i + 1));
    }
    later(() => finish(n, stake), SPIN_MS);
  };
  ...
  return (
    <section className="roulette-hall">
      ...
      <div className="rl-grid">
        <Wheel spinning={spinning} angle={angle} ballPocket={ballPocket} rnum={rnum} won={won} />
